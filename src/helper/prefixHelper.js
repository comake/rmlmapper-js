...
const replacePrefixIfExists = (input, prefixes) => {
  if (input.indexOf(':') !== -1) {
    ...
    if (p.length !== 2) {
      ...
      throw new Error(`replacePrefixIfExists(): Error during processing string: too many ":" in ${input} ! only one allowed.`);
    }
    const prefix = p[0];
    const value = p[1];
    if (!prefixes[prefix]) {
      // prefix not declared
      throw new Error(`replacePrefixIfExists(): Error during processing string: Prefix ${prefix} is not defined`);
    }
    return value;
  }
  return input;
